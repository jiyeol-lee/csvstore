...
func TestInsert(t *testing.T) {
	...
	err = store.Insert(tableName, record)
	if err != nil {
	...
	if result.Records[0]["name"] != "John Doe" {
		t.Errorf("Expected name 'John Doe', got '%s'", result.Records[0]["name"])
	}

	// Check that created_at was automatically added
	if result.Records[0]["created_at"] == "" {
		t.Error("Expected created_at to be automatically set")
	}

	// Check that updated_at was automatically added
	if result.Records[0]["updated_at"] == "" {
...
func TestEdgeCases(t *testing.T) {
	...
	err = store.Insert("nonexistent", CSVRecord{"id": "1"})
	if err == nil {
